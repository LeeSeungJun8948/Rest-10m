...
public class CommonExcelView extends AbstractXlsxView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		Cell cell;
		int rowNum = 0;
		String file_name = (String) model.get("filename") + ".xlsx";
		response.setHeader("Content-Disposition", "attachment; filename=\"" + file_name + "\"");
		// header 출력
		String[] headers = (String[]) model.get("headers");
		HashMap<String, Object> headerMap = (HashMap<String, Object>) model.get("headerMap");
		if (headers != null) {
			...
			for (String header : headerMap.keySet()) {
				row.createCell(colNum++).setCellValue((String) headerMap.get(header));
			}
		}
		// body 출력
